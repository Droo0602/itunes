  ...
  //Also note that that method should be retuning a promise, so you could use .then in this function.
    ...
    $scope.getSongData = function(){
      ...
      itunesService.getSongs($scope.artist).then(function(songs){
        // console.log(songs);
        // for (var i = 0; i < songs.data.results.length; i++){
        //   easyRead[i].Artist = songs.data.results[i].artistName;
        //   easyRead[i].Collection = songs.data.results[i].collectionName;
        //   easyRead[i].AlbumArt = songs.data.results[i].artworkUrl100;
        //   easyRead[i].Type = songs.data.results[i].kind;
        //   easyRead[i].CollectionPrice = songs.data.results[i].collectionPrice;
        // }
        // console.log(easyRead);

        var correctlyFormatedSongs = songs.map(mapItunesSongIntoGridFormat);
        $scope.songData = correctlyFormatedSongs;
      })
    }

    function mapItunesSongIntoGridFormat(itunesSong){
      return {
        Artist: itunesSong.artistName,
        Collection: itunesSong.collectionName,
        AlbumArt: itunesSong.artworkUrl100,
        Type: itunesSong.kind,
        CollectionPrice: itunesSong.collectionPrice
      }
  }
